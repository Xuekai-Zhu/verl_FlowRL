...
class FlowRLActor(DataParallelPPOActor):
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if flowrl_objective == 'vanilla':
                        # FlowRL with importance weight clipping (max=10), using reference policy
                        ...
                        # FlowRL using old policy instead of reference policy
                        policy_loss, flowrl_metrics = self.compute_flowrl_with_old_policy(
                            log_prob=log_prob,
                            ref_log_prob=ref_log_prob,
                            old_log_prob=old_log_prob,
                            log_z=log_z,
                        ...
                        # FlowRL without log Z (ablation study)
                        policy_loss, flowrl_metrics = self.compute_flowrl_no_log_z(
                            log_prob=log_prob,
                            ref_log_prob=ref_log_prob,
                            old_log_prob=old_log_prob,
                            reward=advantages,
    ...
    def compute_flowrl(self,
        ...
        """
        FlowRL objective with importance weight clipping (max=10).
        """
    ...
    def compute_flowrl_with_old_policy(self,
                                log_prob=None,
                                ref_log_prob=None,
                                old_log_prob=None,
                                log_z=None,
        ...
        Exactly the same as compute_flowrl but with ref_log_prob replaced by old_log_prob.
        """
        log_z = log_z.squeeze(-1)

        # Average token log-probs & rewards over valid positions
        avg_log_prob = verl_F.masked_mean(log_prob, response_mask, axis=1)
        avg_old_log_prob = verl_F.masked_mean(old_log_prob, response_mask, axis=1)  # ONLY CHANGE: use old instead of ref
        seq_log_reward = verl_F.masked_mean(reward, response_mask, axis=1)

        # FlowRL residual: logZ + logpf - β*R - log_old (ONLY CHANGE: old instead of ref)
        delta = log_z + avg_log_prob - self.flowrl_beta_coef * seq_log_reward - avg_old_log_prob
    ...
    def compute_flowrl_no_log_z(self,
                                log_prob=None,
                                ref_log_prob=None,
                                old_log_prob=None,
                                reward=None,
        ...
        This version removes the partition function estimation from the objective,
        simplifying the residual to: logpf - β*R - logpref
        """
        # Average token log-probs & rewards over valid positions
        avg_log_prob = verl_F.masked_mean(log_prob, response_mask, axis=1)
        avg_ref_log_prob = verl_F.masked_mean(ref_log_prob, response_mask, axis=1)
